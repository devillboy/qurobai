...
async function generateImage(prompt: string, supabase: any, userId?: string): Promise<string> {
  ...
  if (FIREWORKS_API_KEY) {
    try {
      console.log("Generating image via QurobAi renderer");
      ...
      if (resp.ok) {
        ...
        let imageUrlResult = `data:image/png;base64,${btoa(binary)}`;
        if (userId) {
          try {
      ...
      console.error("QurobAi image renderer error:", resp.status, errTxt);
      if (resp.status === 429) return "Image generation is busy right now. Please try again in a few seconds.";
    } catch (e) {
      console.error("QurobAi image generation error:", e);
    }
  }
